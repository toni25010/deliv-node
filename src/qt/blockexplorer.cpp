...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::DELIV, nValue);
...
{
    std::string genesisblockhash = "000000e62740a0b921234d039d1cc4102c9065c0ff3fca6695fb07bc25cd0d68";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (DeliV.conf).");
            QMessageBox::warning(this, "DeliV Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
