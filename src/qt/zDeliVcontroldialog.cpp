...
#include "ui_zDeliVcontroldialog.h"
...
std::list<std::string> ZDeliVControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZDeliVControlDialog::listMints;

ZDeliVControlDialog::ZDeliVControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZDeliVControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZDeliVControlDialog::~ZDeliVControlDialog()
{
    delete ui;
}

void ZDeliVControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZDeliVControlDialog::updateList()
{
...
void ZDeliVControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZDeliVControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZDeliV_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZDeliVControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZDeliVControlDialog::GetSelectedMints()
{
...
void ZDeliVControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
