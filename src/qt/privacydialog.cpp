                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zDELIV ought to be enough for anybody." - Bill Gates, 2017
    ui->zDELIVpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzDELIVSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzDELIV->setEnabled(false);
        ui->pushButtonMintzDELIV->setToolTip(tr("zDELIV is currently disabled due to maintenance."));

        ui->pushButtonSpendzDELIV->setEnabled(false);
        ui->pushButtonSpendzDELIV->setToolTip(tr("zDELIV is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zDELIVpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzDELIV_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zDELIV is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zDELIV...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zDELIV in ") +
...
void PrivacyDialog::on_pushButtonSpendzDELIV_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zDELIV is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zDELIV
        sendzDELIV();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zDELIV
    sendzDELIV();
}

void PrivacyDialog::on_pushButtonZDeliVControl_clicked()
{
    ZDeliVControlDialog* zDeliVControl = new ZDeliVControlDialog(this);
    zDeliVControl->setModel(walletModel);
    zDeliVControl->exec();
}

void PrivacyDialog::setZDeliVControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzDeliVSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzDELIV()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid DeliV Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zDELIVpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zDELIV is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " DELIV </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zDELIV</b>";
    ...
    // use mints from zDeliV selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZDeliVControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZDeliVControlDialog::GetSelectedMints();
    }
    ...
    // Clear zDeliV selector in case it was used
    ZDeliVControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zDeliV Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " DeliV, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zDeliV Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zDELIV </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zDELIV "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zDELIV "));
    ui->labelzDELIVAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzDELIVSyncStatus->setVisible(fShow);
}
